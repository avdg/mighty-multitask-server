 ...
 *
 * @throws {Error} If `config.resourceServePath` is not provided or if `config.urlPrefixes` exists but is not an array.
 *
 * @returns {(req:IncomingMessage, res:ServerResponse, state:Object) => Promise<boolean>} An async handler function. The handler returns `true` when it handled the request (a response was sent), or `false` when it did not handle the request.
 */
